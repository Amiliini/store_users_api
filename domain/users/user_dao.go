...
func (user *User) Save() *errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to insert id aftert creating a new user", err)
		return errors.NewInternalServerError("database error")
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	...
	for rows.Next() {
		var user User
		//pass a pointer to the sql database, other you will end up with a empty user..
	...
	if len(results) == 0 {
		return nil, errors.NewNotFoundError(fmt.Sprintf("no users matchingstatus %s", status))
	}
	return results, nil
